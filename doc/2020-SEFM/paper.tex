...
\noindent These express \eqnref{eq:vipEnvInit} that the enemy UAV starts in region 4; 
...
\eqnref{eq:vipEnvLiveness} that the enemy UAV must infinitely often go to regions 3, 4, and 5. 
...
\noindent These express \eqnref{eq:vipSysInit} that the VIP starts in region 2, surveillance UAV 1 in region 3, and surveillance UAV 2 in region 5; 
...
As discussed in \sectref{sec:vipExample}, there were two example controllers\footnote{Salty's vip\_orig.salt and Anzu's arbiter.salt} 
...
We had previously run the Salty-synthesized controller in Python for the example in \sectref{sec:vipExample} and not noticed the error, 
