...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\title{\large Experience Report: End-to-End Verification of Initial and Transition Properties of GR(1) Designs in SPARK\thanks{Supported by AFRL contract FA8650-16-C-2642 and AFOSR grant RQCOR20-35.}}
%
...
In \sectref{sec:caseStudies}, we discuss our results on examples pulled from various sources.
...
Analysis with SPARK did reveal unexpected behaviors in two example controllers. 
...
environment transition terms that allowed the environment to produce an input that would cause 
the environment to violate its own specification on the next time step. 
To explain, suppose $p$ is an input. 
Then these specifications had the form $\always \lnot p$ instead of $\always \lnext \lnot p$. 
The distinction is important.
In GR(1), specifications are interpreted in the context of a two-player game in which transition specifications 
are over the current state and the next state. 
