...
A formula of the form $\eventually b$ holds if $b$ is eventually true at some future time step. 
A formula of the form $\always \eventually b$ holds if $b$ is true infinitely often in the future. 
And a formula of the form $\lnext b$ holds if $b$ is true at the next time step. 
...
Both of these are declared (lines 27-30) with aspect \lstinline{Ghost}, indicating that they are intended mainly for proof purposes, 
...
In this case, removing the ghost code can save a lot of memory, 
...
since \lstinline{tick = False} as the system enters state 1, and \lstinline{State_To_Output_Equivalence} returns \lstinline{System'(red => True, yellow => False, green => False)}, 
...
meaning it is ``ghost code'' used only for proof and will not be executed.
\lstinline{Is_Init} and \lstinline{Env_Init} are used for proof but are also callable in functional code. 
We chose to make them non-ghost functions for reasons related to the meaning of GR(1) specifications. 
