...
In the private part of the specification, \lstinline{Controller} (lines 34-39) is a record that stores the controller's internal state number, 
...
When there are multiple inputs or outputs, they are wrapped in a record of type \lstinline{System} (lines 4-6) or \lstinline{Environment}, respectively. 
...
with examples with unproved functional contracts marked in red and examples that could not be analyzed set to -10 in red.
...
Encoding the the lookup table as a case statement is more straightforward, because for instance, 
