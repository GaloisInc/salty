...
LRH 30 Aug 2019:
Modified from TuLiP repository to produce a Salty specification.
Ensures all specifications are realizable.
"""
...
salty_file.write('input yc : Sc = Y' + re.search("Y_r\s+=\s+(\d+)", spec.sys_init[0]).group(1) + 'c\n')
salty_file.write('input yr : Sr = Y' + re.search("Y_c\s+=\s+(\d+)", spec.sys_init[0]).group(1) + 'r\n')
...
# obj = re.search('X_0_c\w+\(0,\w*(\d+)', spec)
# obj = re.search('X_0_r\w+\(0,\w*(\d+)', str)
salty_file.close()
